...
두 개의 값만을 가질 수 있는 이항반응변수를 모형 \@ref(eq:linear-reg)으로 설명할 때 발생할 수 있는 첫 번쨰 문제는 오차항 $\varepsilon$ 이 $N(0,\sigma^{2})$ 의 분포를 한다는 가정을 만족시킬 수 없다는 것이다. 
...
p1 <- base + 
  stat_function(geom = "line", fun = f, args = list(b0 = -4, b1 = 1), size = 1, 
                aes(color = "b0 = -4,  ")) +
  stat_function(geom = "line", fun = f, args = list(b0 = 0, b1 = 1), size = 1,
                aes(color = "b0 = 0,   ")) +
  stat_function(geom = "line", fun = f, args = list(b0 = 4, b1 = 1), size = 1,
                aes(color = "b0 = 4")) +
...
p2 <- base + 
  stat_function(geom = "line", fun = f, args = list(b0 = 0, b1 = 0.25), size = 1, 
                aes(color = "b1 = 0.25,  ")) +
  stat_function(geom = "line", fun = f, args = list(b0 = 0, b1 = 0.5), size = 1,
                aes(color = "b1 = 0.5,  ")) +
  stat_function(geom = "line", fun = f, args = list(b0 = 0, b1 = 1), size = 1,
                aes(color = "b1 = 1")) +
  labs(x = "X", y = "P(Y = 1)", color = NULL) +
...
Default %>% 
  ...
  ggplot(aes(x = balance, y = as.numeric(default)-1)) +
  geom_jitter(height = 0.005, width = 100) +
  geom_line(aes(x = balance, y = pred, color = student), size = 1) +
  labs(y = "Probability of default")
```
...
df1 %>% 
    mutate(p1 = predict(fit1, newdata = df1, type = "response")) %>% 
    ggplot() +
    geom_col(aes(x = k5, y = p1, fill = wc), position = "dodge") +
    labs(y = "Prob") 
```
...
df2 %>% 
  mutate(p2 = predict(fit1, newdata = df2, type = "response")) %>% 
  ggplot() +
  geom_line(aes(x = lwg, y = p2, color = factor(k5)), size = 1) +
  facet_wrap(vars(wc)) +
...
pROC::roc(Mroz$lfp, predict(fit1, type = "response"), plot = TRUE)
```
...
```{r}
set.seed(123)
x.id <- createDataPartition(Caravan$Purchase, p = 0.7, list = FALSE)
...
```{r}
set.seed(123)
x.id <- createDataPartition(test_C$Purchase, p = 0.5, list = FALSE)
